Shader "Quantum Theory/OLD PBR - Parallax Mapping" {
     Properties {
         _MainTex ("Albedo (RGB)", 2D) = "white" {}
         _BumpMap("NormalMap", 2D) = "white" {}
         _MetallicGlossMap ("Metallic (R) AO (G) Height (B) Smoothness (A)", 2D) = "white" {}
        // _DetailNormal ("Detail NormalMap", 2D) = "white" {}
       //  _DetailAlbedo ("Detail Albedo", 2D) = "white" {}
         
         _HeightScale ("Height Scale", Range(0,.35)) = 0.2
    
         _TotalHeightBias ("TotalHeightBias", Range(0,1)) = 1.0
     ...
     SubShader {
		...
		fixed4 _Color;
 			...
 			out float maskResult)
 		{
 			...
 			//float3 detailAlbedo = tex2D(_DetailAlbedo, detailAlbedoUvs + baseOffsets * _DetailOffsetScale);
			...
			albedoResult.xyz = albedoResult.xyz;// *detailAlbedo.xyz * 2;
 			...
 			//float3 detailNormal = UnpackNormal(tex2D(_DetailNormal, detailNormalUvs + baseOffsets * _DetailOffsetScale)).xyz;
		...
		//	float2 detailNormalMapUvs = IN.uv_DetailNormal;
			float2 mhasUvs = IN.uv_MetallicGlossMap;
			//float2 detailAlbedoUvs = IN.uv_DetailNormal;
		...
		//	float3 detailNormal = float3(1,1,0) * UnpackNormal(tex2D(_DetailNormal, detailNormalMapUvs)).xyz;
			previousNormal = normalize(previousNormal);// +detailNormal);
			
			float4 previousMetAndSmoothness = tex2D(_MetallicGlossMap, mhasUvs);
			
			//float3 detailAlbedo = tex2D(_DetailAlbedo, detailAlbedoUvs);
